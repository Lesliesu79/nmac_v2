...
static int nmac_eq_int(struct notifier_block *nb, unsigned long action, void *data)
{
	struct nmac_eq_ring *ring = container_of(nb, struct nmac_eq_ring, irq_nb);

	nmac_process_eq(ring);
	nmac_arm_eq(ring);
		...
		int index, u8 __iomem *hw_addr)
{
	...
	*ring_ptr = ring;
	...
	ring->irq_nb.notifier_call = nmac_eq_int;

	ring->hw_addr = hw_addr;
	ring->hw_ptr_mask = 0xffff;
	ring->hw_head_ptr = hw_addr + NMAC_EVENT_QUEUE_HEAD_PTR_REG;
	ring->hw_tail_ptr = hw_addr + NMAC_EVENT_QUEUE_TAIL_PTR_REG;

	ring->head_ptr = 0;
	ring->tail_ptr = 0;

	// deactivate queue
	iowrite32(0, ring->hw_addr + NMAC_EVENT_QUEUE_ACTIVE_LOG_SIZE_REG);
...
{
	struct nmac_eq_ring *ring = *ring_ptr;

	nmac_free_eq_ring(ring);
...
int nmac_alloc_eq_ring(struct nmac_eq_ring *ring, int size, int stride)
{
	...
	iowrite32(0, ring->hw_addr + NMAC_EVENT_QUEUE_ACTIVE_LOG_SIZE_REG);
	// set base address
	iowrite32(ring->buf_dma_addr, ring->hw_addr + NMAC_EVENT_QUEUE_BASE_ADDR_REG + 0);
	iowrite32(ring->buf_dma_addr >> 32, ring->hw_addr + NMAC_EVENT_QUEUE_BASE_ADDR_REG + 4);
	// set interrupt index
	iowrite32(0, ring->hw_addr + NMAC_EVENT_QUEUE_INTERRUPT_INDEX_REG);
	// set pointers
	iowrite32(ring->head_ptr & ring->hw_ptr_mask, ring->hw_addr + NMAC_EVENT_QUEUE_HEAD_PTR_REG);
	iowrite32(ring->tail_ptr & ring->hw_ptr_mask, ring->hw_addr + NMAC_EVENT_QUEUE_TAIL_PTR_REG);
	// set size
	iowrite32(ilog2(ring->size), ring->hw_addr + NMAC_EVENT_QUEUE_ACTIVE_LOG_SIZE_REG);
...
int nmac_activate_eq_ring(struct nmac_eq_ring *ring, struct nmac_irq *irq)
{
	...
	iowrite32(ring->buf_dma_addr >> 32, ring->hw_addr + NMAC_EVENT_QUEUE_BASE_ADDR_REG + 4);
	// set interrupt index
	iowrite32(ring->irq_index, ring->hw_addr + NMAC_EVENT_QUEUE_INTERRUPT_INDEX_REG);
	// set pointers
	iowrite32(ring->head_ptr & ring->hw_ptr_mask,
			ring->hw_addr + NMAC_EVENT_QUEUE_HEAD_PTR_REG);
	iowrite32(ring->tail_ptr & ring->hw_ptr_mask,
			ring->hw_addr + NMAC_EVENT_QUEUE_TAIL_PTR_REG);
	// set size and activate queue
	iowrite32(ilog2(ring->size) | NMAC_EVENT_QUEUE_ACTIVE_MASK,
			ring->hw_addr + NMAC_EVENT_QUEUE_ACTIVE_LOG_SIZE_REG);
...
void nmac_arm_eq(struct nmac_eq_ring *ring)
{
	...
	iowrite32(ring->irq_index | NMAC_EVENT_QUEUE_ARM_MASK,
			ring->hw_addr + NMAC_EVENT_QUEUE_INTERRUPT_INDEX_REG);
}

void nmac_process_eq(struct nmac_eq_ring *eq_ring)
{
	struct nmac_if *interface = eq_ring->interface;
	struct nmac_event *event;
	...
	nmac_eq_read_head_ptr(eq_ring);

	eq_tail_ptr = eq_ring->tail_ptr;
	eq_index = eq_tail_ptr & eq_ring->size_mask;

	while (eq_ring->head_ptr != eq_tail_ptr) {
		event = (struct nmac_event *)(eq_ring->buf + eq_index * eq_ring->stride);

		if (event->type == NMAC_EVENT_TYPE_TX_CPL) {
	...
	nmac_eq_write_tail_ptr(eq_ring);
}
