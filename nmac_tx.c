...
{
	nmac_deactivate_tx_ring(ring);

	if (ring->buf) {
		nmac_free_tx_buf(ring);
...
{
	struct nmac_tx_info *tx_info = &ring->tx_info[index];
...
int nmac_free_tx_buf(struct nmac_ring *ring)
{
	...
	while (!nmac_is_tx_ring_empty(ring)) {
		index = ring->clean_tail_ptr & ring->size_mask;
		nmac_free_tx_desc(ring, index, 0);
		ring->clean_tail_ptr++;
		cnt++;
	}
...
int nmac_process_tx_cq(struct nmac_cq_ring *cq_ring, int napi_budget)
{
	...
	struct nmac_ring *tx_ring = cq_ring->src_ring;
	struct nmac_priv *priv = tx_ring->priv;
	...
	while (cq_ring->head_ptr != cq_tail_ptr && done < budget) {
		...
		if (unlikely(tx_info->ts_requested)) {
			...
			hwts.hwtstamp = nmac_read_cpl_ts(interface->mdev, tx_ring, cpl);
			skb_tstamp_tx(tx_info->skb, &hwts);
		}
		// free TX descriptor
		nmac_free_tx_desc(tx_ring, ring_index, napi_budget);
	...
	if (netif_tx_queue_stopped(tx_ring->tx_queue) && !nmac_is_tx_ring_full(tx_ring))
...
int nmac_poll_tx_cq(struct napi_struct *napi, int budget)
{
	...
	done = nmac_process_tx_cq(cq_ring, budget);
...
static bool nmac_map_skb(struct nmac_ring *ring, struct nmac_tx_info *tx_info,
		struct nmac_desc *tx_desc, struct sk_buff *skb)
{
...
netdev_tx_t nmac_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	struct skb_shared_info *shinfo = skb_shinfo(skb);
	struct nmac_priv *priv = netdev_priv(ndev);
	struct nmac_ring *ring;
	struct nmac_tx_info *tx_info;
	struct nmac_desc *tx_desc;
	int ring_index;
	u32 index;
	bool stop_queue;
	u32 clean_tail_ptr;
	...
	tx_desc = (struct nmac_desc *)(ring->buf + index * ring->stride);

	tx_info = &ring->tx_info[index];

	// TX hardware timestamp
	tx_info->ts_requested = 0;
	if (unlikely(priv->if_features & NMAC_IF_FEATURE_PTP_TS && shinfo->tx_flags & SKBTX_HW_TSTAMP)) {
	...
	// map skb
	if (!nmac_map_skb(ring, tx_info, tx_desc, skb))
		// map failed
		goto tx_drop_count;
	...
	stop_queue = nmac_is_tx_ring_full(ring);
	...
	if (unlikely(stop_queue)) {
		...
		if (unlikely(!nmac_is_tx_ring_full(ring)))
