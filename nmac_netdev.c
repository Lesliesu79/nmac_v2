...
static int nmac_start_port(struct net_device *ndev)
{
	...
	for (k = 0; k < min(priv->rx_queue_count, priv->rx_cpl_queue_count); k++) {
		...
		nmac_activate_rx_ring(priv->rx_ring[k], priv, priv->rx_cpl_ring[k]);
	}

	// set up TX queues
	for (k = 0; k < min(priv->tx_queue_count, priv->tx_cpl_queue_count); k++) {
		// set up CQ
		nmac_activate_cq_ring(priv->tx_cpl_ring[k],
				priv->event_ring[k % priv->event_queue_count]);

		netif_tx_napi_add(ndev, &priv->tx_cpl_ring[k]->napi,
				nmac_poll_tx_cq, NAPI_POLL_WEIGHT);
		napi_enable(&priv->tx_cpl_ring[k]->napi);

		nmac_arm_cq(priv->tx_cpl_ring[k]);

		// set up queue
		priv->tx_ring[k]->tx_queue = netdev_get_tx_queue(ndev, k);
		nmac_activate_tx_ring(priv->tx_ring[k], priv, priv->tx_cpl_ring[k]);
	}

	// set MTU
	nmac_interface_set_tx_mtu(priv->interface, ndev->mtu + ETH_HLEN);
	nmac_interface_set_rx_mtu(priv->interface, ndev->mtu + ETH_HLEN);

	// configure RSS
	nmac_interface_set_rx_queue_map_rss_mask(priv->interface, 0, rounddown_pow_of_two(priv->rx_queue_count)-1);

	// enable first scheduler
	nmac_activate_sched_block(priv->sched_block[0]);
	...
	if (nmac_link_status_poll) {
		priv->link_status = 0;
		mod_timer(&priv->link_status_timer,
				jiffies + msecs_to_jiffies(nmac_link_status_poll));
	} else {
...
static int nmac_stop_port(struct net_device *ndev)
{
	struct nmac_priv *priv = netdev_priv(ndev);
	struct nmac_dev *mdev = priv->mdev;
	int k;
...
//		netif_device_detach(ndev);
	...
	nmac_update_stats(ndev);
	priv->port_up = false;
	spin_unlock_bh(&priv->stats_lock);

	// disable schedulers
	for (k = 0; k < priv->sched_block_count; k++)
		nmac_deactivate_sched_block(priv->sched_block[k]);

	// deactivate TX queues
	for (k = 0; k < min(priv->tx_queue_count, priv->tx_cpl_queue_count); k++) {
		napi_disable(&priv->tx_cpl_ring[k]->napi);

		nmac_deactivate_tx_ring(priv->tx_ring[k]);

		nmac_deactivate_cq_ring(priv->tx_cpl_ring[k]);
	...
	for (k = 0; k < min(priv->rx_queue_count, priv->rx_cpl_queue_count); k++) {
		...
		nmac_deactivate_rx_ring(priv->rx_ring[k]);

		nmac_deactivate_cq_ring(priv->rx_cpl_ring[k]);
	...
	for (k = 0; k < priv->tx_queue_count; k++)
		nmac_free_tx_buf(priv->tx_ring[k]);

	// free descriptors in RX queues
	for (k = 0; k < priv->rx_queue_count; k++)
		nmac_free_rx_buf(priv->rx_ring[k]);

	netif_carrier_off(ndev);
	return 0;
}

static int nmac_open(struct net_device *ndev)
{
	struct nmac_priv *priv = netdev_priv(ndev);
	struct nmac_dev *mdev = priv->mdev;
	int ret = 0;
...
static int nmac_close(struct net_device *ndev)
{
	struct nmac_priv *priv = netdev_priv(ndev);
	struct nmac_dev *mdev = priv->mdev;
	int ret = 0;
...
void nmac_update_stats(struct net_device *ndev)
{
	...
	bytes = 0;
	for (k = 0; k < priv->rx_queue_count; k++) {
		const struct nmac_ring *ring = priv->rx_ring[k];
	...
	bytes = 0;
	for (k = 0; k < priv->tx_queue_count; k++) {
		const struct nmac_ring *ring = priv->tx_ring[k];
...
static void nmac_get_stats64(struct net_device *ndev,
...
static int nmac_hwtstamp_set(struct net_device *ndev, struct ifreq *ifr)
{
	struct nmac_priv *priv = netdev_priv(ndev);
...
static int nmac_hwtstamp_get(struct net_device *ndev, struct ifreq *ifr)
{
	struct nmac_priv *priv = netdev_priv(ndev);
...
static int nmac_change_mtu(struct net_device *ndev, int new_mtu)
{
	struct nmac_priv *priv = netdev_priv(ndev);
	struct nmac_dev *mdev = priv->mdev;
...
static int nmac_ioctl(struct net_device *ndev, struct ifreq *ifr, int cmd)
{
	switch (cmd) {
	case SIOCSHWTSTAMP:
		return nmac_hwtstamp_set(ndev, ifr);
	case SIOCGHWTSTAMP:
		return nmac_hwtstamp_get(ndev, ifr);
	default:
		return -EOPNOTSUPP;
	}
}

static const struct net_device_ops nmac_netdev_ops = {
	.ndo_open = nmac_open,
	.ndo_stop = nmac_close,
	.ndo_start_xmit = nmac_start_xmit,
	.ndo_get_stats64 = nmac_get_stats64,
	.ndo_validate_addr = eth_validate_addr,
	.ndo_change_mtu = nmac_change_mtu,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	.ndo_eth_ioctl = nmac_ioctl,
#else
	.ndo_do_ioctl = nmac_ioctl,
#endif
};

static void nmac_link_status_timeout(struct timer_list *timer)
{
	struct nmac_priv *priv = from_timer(priv, timer, link_status_timer);
	struct nmac_if *interface = priv->interface;
	int k;
	unsigned int up;

	// "combine" all TX/RX status signals of all ports of this interface
	for (k = 0, up = 0; k < interface->port_count; k++) {
		if (!(nmac_port_get_tx_status(interface->port[k]) & 0x1))
			continue;
		if (!(nmac_port_get_rx_status(interface->port[k]) & 0x1))
			continue;
	...
	mod_timer(&priv->link_status_timer, jiffies + msecs_to_jiffies(nmac_link_status_poll));
}

int nmac_create_netdev(struct nmac_if *interface, struct net_device **ndev_ptr,
		int index, int dev_port)
{
	struct nmac_dev *mdev = interface->mdev;
	...
	ndev = alloc_etherdev_mqs(sizeof(*priv), NMAC_MAX_TX_RINGS, NMAC_MAX_RX_RINGS);
	if (!ndev) {
	...
	*ndev_ptr = ndev;
	...
	memset(priv, 0, sizeof(struct nmac_priv));
	...
	for (k = 0; k < priv->tx_queue_count; k++) {
		ret = nmac_alloc_tx_ring(priv->tx_ring[k], nmac_num_tx_queue_entries,
				NMAC_DESC_SIZE * desc_block_size);
		if (ret)
			goto fail;
	}

	for (k = 0; k < priv->tx_cpl_queue_count; k++) {
		ret = nmac_alloc_cq_ring(priv->tx_cpl_ring[k], nmac_num_tx_queue_entries,
				NMAC_CPL_SIZE);
		if (ret)
			goto fail;
	}
	...
	for (k = 0; k < priv->rx_queue_count; k++) {
		ret = nmac_alloc_rx_ring(priv->rx_ring[k], nmac_num_rx_queue_entries,
				NMAC_DESC_SIZE);
		if (ret)
			goto fail;
	}
	...
	for (k = 0; k < priv->rx_cpl_queue_count; k++) {
		ret = nmac_alloc_cq_ring(priv->rx_cpl_ring[k], nmac_num_rx_queue_entries,
				NMAC_CPL_SIZE);
	...
	// entry points
	ndev->netdev_ops = &nmac_netdev_ops;
	ndev->ethtool_ops = &nmac_ethtool_ops;

	// set up features
	ndev->hw_features = NETIF_F_SG;

	if (priv->if_features & NMAC_IF_FEATURE_RX_CSUM)
		ndev->hw_features |= NETIF_F_RXCSUM;

	if (priv->if_features & NMAC_IF_FEATURE_TX_CSUM)
	...
	netif_carrier_off(ndev);
	if (nmac_link_status_poll)
		timer_setup(&priv->link_status_timer, nmac_link_status_timeout, 0);
