#include "nmac.h"

int nmac_create_cq_ring(struct nmac_if *interface, struct nmac_cq_ring **ring_ptr,
		int index, u8 __iomem *hw_addr)
{
	struct nmac_cq_ring *ring;

	ring = kzalloc(sizeof(*ring), GFP_KERNEL);
	if (!ring)
		return -ENOMEM;

	*ring_ptr = ring;

	ring->dev = interface->dev;
	ring->interface = interface;

	ring->index = index;
	ring->active = 0;

	ring->hw_addr = hw_addr;
	ring->hw_ptr_mask = 0xffff;
	ring->hw_head_ptr = hw_addr + NMAC_CPL_QUEUE_HEAD_PTR_REG;
	ring->hw_tail_ptr = hw_addr + NMAC_CPL_QUEUE_TAIL_PTR_REG;

	ring->head_ptr = 0;
	ring->tail_ptr = 0;

	// deactivate queue
	iowrite32(0, ring->hw_addr + NMAC_CPL_QUEUE_ACTIVE_LOG_SIZE_REG);

	return 0;
}

void nmac_destroy_cq_ring(struct nmac_cq_ring **ring_ptr)
{
	struct nmac_cq_ring *ring = *ring_ptr;

	nmac_free_cq_ring(ring);

	*ring_ptr = NULL;
	kfree(ring);
}

int nmac_alloc_cq_ring(struct nmac_cq_ring *ring, int size, int stride)
{
	if (ring->active || ring->buf)
		return -EINVAL;

	ring->size = roundup_pow_of_two(size);
	ring->size_mask = ring->size - 1;
	ring->stride = roundup_pow_of_two(stride);

	ring->buf_size = ring->size * ring->stride;
	ring->buf = dma_alloc_coherent(ring->dev, ring->buf_size, &ring->buf_dma_addr, GFP_KERNEL);
	if (!ring->buf)
		return -ENOMEM;

	ring->head_ptr = 0;
	ring->tail_ptr = 0;

	// deactivate queue
	iowrite32(0, ring->hw_addr + NMAC_CPL_QUEUE_ACTIVE_LOG_SIZE_REG);
	// set base address
	iowrite32(ring->buf_dma_addr, ring->hw_addr + NMAC_CPL_QUEUE_BASE_ADDR_REG + 0);
	iowrite32(ring->buf_dma_addr >> 32, ring->hw_addr + NMAC_CPL_QUEUE_BASE_ADDR_REG + 4);
	// set interrupt index
	iowrite32(0, ring->hw_addr + NMAC_CPL_QUEUE_INTERRUPT_INDEX_REG);
	// set pointers
	iowrite32(ring->head_ptr & ring->hw_ptr_mask, ring->hw_addr + NMAC_CPL_QUEUE_HEAD_PTR_REG);
	iowrite32(ring->tail_ptr & ring->hw_ptr_mask, ring->hw_addr + NMAC_CPL_QUEUE_TAIL_PTR_REG);
	// set size
	iowrite32(ilog2(ring->size), ring->hw_addr + NMAC_CPL_QUEUE_ACTIVE_LOG_SIZE_REG);

	return 0;
}

void nmac_free_cq_ring(struct nmac_cq_ring *ring)
{
	nmac_deactivate_cq_ring(ring);

	if (ring->buf) {
		dma_free_coherent(ring->dev, ring->buf_size, ring->buf, ring->buf_dma_addr);
		ring->buf = NULL;
		ring->buf_dma_addr = 0;
	}
}

int nmac_activate_cq_ring(struct nmac_cq_ring *ring, struct nmac_eq_ring *eq_ring)
{
	nmac_deactivate_cq_ring(ring);

	if (!ring->buf || !eq_ring)
		return -EINVAL;

	ring->eq_ring = eq_ring;
	ring->eq_index = eq_ring->index;

	// deactivate queue
	iowrite32(0, ring->hw_addr + NMAC_CPL_QUEUE_ACTIVE_LOG_SIZE_REG);
	// set base address
	iowrite32(ring->buf_dma_addr, ring->hw_addr + NMAC_CPL_QUEUE_BASE_ADDR_REG + 0);
	iowrite32(ring->buf_dma_addr >> 32, ring->hw_addr + NMAC_CPL_QUEUE_BASE_ADDR_REG + 4);
	// set interrupt index
	iowrite32(ring->eq_index, ring->hw_addr + NMAC_CPL_QUEUE_INTERRUPT_INDEX_REG);
	// set pointers
	iowrite32(ring->head_ptr & ring->hw_ptr_mask, ring->hw_addr + NMAC_CPL_QUEUE_HEAD_PTR_REG);
	iowrite32(ring->tail_ptr & ring->hw_ptr_mask, ring->hw_addr + NMAC_CPL_QUEUE_TAIL_PTR_REG);
	// set size and activate queue
	iowrite32(ilog2(ring->size) | NMAC_CPL_QUEUE_ACTIVE_MASK,
			ring->hw_addr + NMAC_CPL_QUEUE_ACTIVE_LOG_SIZE_REG);

	ring->active = 1;

	return 0;
}

void nmac_deactivate_cq_ring(struct nmac_cq_ring *ring)
{
	// deactivate queue
	iowrite32(ilog2(ring->size), ring->hw_addr + NMAC_CPL_QUEUE_ACTIVE_LOG_SIZE_REG);
	// disarm queue
	iowrite32(ring->eq_index, ring->hw_addr + NMAC_CPL_QUEUE_INTERRUPT_INDEX_REG);

	ring->eq_ring = NULL;

	ring->active = 0;
}

bool nmac_is_cq_ring_empty(const struct nmac_cq_ring *ring)
{
	return ring->head_ptr == ring->tail_ptr;
}

bool nmac_is_cq_ring_full(const struct nmac_cq_ring *ring)
{
	return ring->head_ptr - ring->tail_ptr >= ring->size;
}

void nmac_cq_read_head_ptr(struct nmac_cq_ring *ring)
{
	ring->head_ptr += (ioread32(ring->hw_head_ptr) - ring->head_ptr) & ring->hw_ptr_mask;
}

void nmac_cq_write_tail_ptr(struct nmac_cq_ring *ring)
{
	iowrite32(ring->tail_ptr & ring->hw_ptr_mask, ring->hw_tail_ptr);
}

void nmac_arm_cq(struct nmac_cq_ring *ring)
{
	if (!ring->active)
		return;

	iowrite32(ring->eq_index | NMAC_CPL_QUEUE_ARM_MASK,
			ring->hw_addr + NMAC_CPL_QUEUE_INTERRUPT_INDEX_REG);
}
